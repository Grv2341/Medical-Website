...
<body>
	...
	<div id="body">
		...
		<div id="right">
			<?php
			$pid=$_GET['pid'];
		$sql="SELECT * FROM prescription where pid='$pid' and status=0 ORDER BY datep DESC ";
		...
		while($row = $result->fetch_assoc()){
			if($id!=$row['prespid']){
				if($count<2){
					echo '</ul>';
					echo '<form action="cpgive.php" method="get">';
					echo '<input type="hidden" name="pid" value="'.$id.'">';
					echo '<button class="visit" type="submit" style="width:200px;">Prescription Given</button>';
					echo '</form>';
					echo '</div>';
				}
				$did=$row['did'];
